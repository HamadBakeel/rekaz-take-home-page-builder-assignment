...
const PropertyEditor: React.FC<PropertyEditorProps> = React.memo(({
    ...
    useEffect(() => {
        if (section && isDirty && !hasValidationErrors && !isUpdatingRef.current) {
    ...
    }, [debouncedProps, section?.id, onSectionUpdate, isDirty, hasValidationErrors])
                ...
                return [
                    ...
                    },
                    {
                        ...
                        validation: (value) => {
                            ...
                            return result.success ? null : { field: 'logoUrl', message: result.error.issues?.[0]?.message || 'Invalid image URL' }
                        }
                    },
                    ...baseFields
                ]
                ...
                return [
                    ...
                    },
                    {
                        ...
                        validation: (value) => {
                            ...
                            return result.success ? null : { field: 'ctaLink', message: 'Invalid URL format' }
                        }
                    },
                    {
                        key: 'backgroundImage',
                        ...
                        validation: (value) => {
                            if (!value) return null
                            const result = ImageUrlSchema.safeParse(value)
                            return result.success ? null : { field: 'backgroundImage', message: result.error.issues?.[0]?.message || 'Invalid image URL' }
                        }
                    },
                    ...baseFields
                ]
                ...
                return [
                    ...
                    },
                    {
                        ...
                        validation: (value) => {
                            if (!value) return null
                            const result = ImageUrlSchema.safeParse(value)
                            return result.success ? null : { field: 'imageUrl', message: result.error.issues?.[0]?.message || 'Invalid image URL' }
                        }
                    },
                    {
                        key: 'imagePosition',
    ...
    const validateField = useCallback((key: string, value: any): ValidationError | null => {
    ...
    const renderField = useCallback((config: FieldConfig) => {
        ...
        switch (config.type) {
            ...
            case 'select':
                return (
                    <Select
                        value={value}
                        onValueChange={(newValue) => handleFieldChange(config.key, newValue)}
                    >
    ...
    }, [localProps, errors, handleFieldChange])
                ...
                {fieldConfigs.map((config) => (
                    <div key={config.key} className="space-y-2">
                        <Label
                            ...
                            className={`text-sm font-medium ${config.required ? 'after:content-["*"] after:text-red-500 after:ml-1' : ''}`}
                        >
                            {config.label}
                        </Label>
                        {renderField(config)}
                        ...
                        <div className="flex items-start justify-between">
                            <div className="flex-1">
                                <p className="text-sm text-red-700 font-medium">
                                    {errors._updateError ? 'Update failed:' : 'Please fix the following errors:'}
                                </p>
